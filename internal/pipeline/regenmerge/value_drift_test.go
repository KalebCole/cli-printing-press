...
func TestClassifyPreservesClientHookNamedDeclarationsOutsideGeneratedRoot(t *testing.T) {
	t.Parallel()

	dir := t.TempDir()
	pubDir := filepath.Join(dir, "pub")
	freshDir := filepath.Join(dir, "fresh")
	rel := filepath.Join("internal", "mcp", "hooks.go")
	for _, root := range []string{pubDir, freshDir} {
		require.NoError(t, os.MkdirAll(filepath.Join(root, "internal", "mcp"), 0o755))
	}
...
func TestClassifyChecksClientHookNamedValuesOutsideGeneratedRoot(t *testing.T) {
	t.Parallel()
...
var clientHooks = []string{"custom"}
`
	fresh := `// Generated by CLI Printing Press. DO NOT EDIT.
package mcp

var clientHooks = []string{"generated"}
`
	require.NoError(t, os.WriteFile(filepath.Join(pubDir, rel), []byte(pub), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(freshDir, rel), []byte(fresh), 0o644))
	...
	require.NoError(t, err)
	got := verdictMap(report)[filepath.ToSlash(rel)]
	assert.Equal(t, VerdictTemplatedValueDrift, got)
}
