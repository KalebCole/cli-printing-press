...
func TestPathsResolverEmitsAndGeneratedCLIBuilds(t *testing.T) {
	...
	require.Contains(t, rootSrc, "type clientHookSurface uint8")
	require.Contains(t, rootSrc, "clientHookSurfaceBoth = clientHookSurfaceCLI | clientHookSurfaceMCP")
	require.Contains(t, rootSrc, "var clientHooks []clientHookRegistration")
	require.Contains(t, rootSrc, "func registerClientHook(hook func(*client.Client) error)")
	require.Contains(t, rootSrc, "registerClientHookFor(clientHookSurfaceCLI, hook)")
	require.Contains(t, rootSrc, "func registerClientHookFor(surface clientHookSurface, hook func(*client.Client) error)")
	require.Contains(t, rootSrc, "func ApplyMCPClientHooks(c *client.Client) error")
	require.Contains(t, rootSrc, "if err := applyClientHooks(clientHookSurfaceCLI, c); err != nil {")
